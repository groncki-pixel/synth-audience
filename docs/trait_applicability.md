...
where $\mathbf{T}_{parent\_avg} = \frac{\sum_{p \in parents} w_p \mathbf{T}_p}{\sum_p w_p}$ and $w_p$ are parent weights (default equal unless household metadata indicates primary caregiver).
